...
class UploadForm extends Model
{
  ...
  public function upload()
  {
    try {
      ...
      if (!$this->mkDirs($path)) {
        throw new \Exception(Yii::$app->language == 'zh-CN' ? '上传目录生成失败！' : 'Upload directory generation failed'); //上传目录生成失败！
      }

      //图片裁剪
      $bigImage = $this->crop();
      //配置大中小图上传路径
      $bigImageUrl = $path . '/' . 'big' . '.' . $this->imageFile->extension;
      $middleImageUrl = $path . '/' . 'middle' . '.' . $this->imageFile->extension;
      $smallImageUrl = $path . '/' . 'small' . '.' . $this->imageFile->extension;
      //生成缩略中小图
      ...
      if (!imagepng($bigImage, $bigImageUrl) || !imagepng($middleImage, $middleImageUrl) || !imagepng($smallImage, $smallImageUrl)) {
        throw new \Exception(Yii::$app->language == 'zh-CN' ? '上传失败！' : 'Upload failed');
      }
  ...
  public function crop()
  {
    ...
    switch ($size['mime']) {
      ...
      case 'image/jpg':
        $dm = imagecreatefromjpeg($this->imageFile->tempName);
        break;
